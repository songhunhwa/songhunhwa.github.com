...
- 비즈니스에 미치는 영향력 고려
- 지표의 타당성과 신뢰성 고려
데이터 분석을 진행할 때, 분석가는 많은 유혹을 느낀다. 시간을 좀더 투자해서 세부적으로 깊게 들어가보고 싶고, 여러 방법론을 적용하면서 성능을 올리고자 하는 생각을 가지게 된다. 또 분석을 진행하다 보면 목적성을 상실하고 분석 행위의 의미를 잃게되는 경험을 할수도 있다. 이 때 나침반의 역할을 하는 것이 목적이다. 문서화를 하여 분석 진행중에 상시로 확인하면 배가 산으로 가는 일을 방지할 수 있다.

#### 프레임 설정
데이터 분석 프로젝트를 시작할 때, (내용이 비어 있는) 틀을 짜고 추후에 데이터 분석을 통해 내용을 채워 넣는 방식으로 진행하는 것이 효율적이다. 사전에 설정한 프레임이 잘못된 방향으로 분석이 진행되는 것으로 예방한다. 더불어 예상 내용을 미리 추측하고 각 결과에 따른 대응 방안을 미리 예상해두면 (생각할 시간이 많기 때문에) 풍부한 인사이트를 도출할 수 있다. 프레임을 설정할 때 아래 원칙을 고려하면 도움이 된다.
- 문제 해결에 도달하기 위해 세부적인 주제 선정
