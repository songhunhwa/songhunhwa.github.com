...
첫번째 항목의 경우 주로 SQL을 활용하며, 다양한 소스(DB, Hadoop 등)로부터 데이터 분석을 위한 기본적인 테이블을 만드는 단계이다. 이 단계에서 가장 중요한 점은 테이블과 컬럼의 명칭, 처리 기준, 조인시 증식 방지 등이며, 데이터 엔지니어로부터 도움이 필요한 경우가 많다.    
...
#### 데이터 분석
분석 영역은 사실 매우 큰 영역을 아우르는 범위이며, 도메인과 여러 상황에 따라 다양한 분석을 진행한다. 간략하게 영역을 구분하면 아래와 같다.
- 지표 개발 및  트래킹
- 통계 분석
- 머신러닝
