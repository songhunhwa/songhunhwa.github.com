...
- 데이터 엔지니어
	- 소프트웨어 개발에 대한 경험과 전문성을 보유하고 있으며, 시스템 환경(Spark, Hadoop) 설치 및 마트 관리와 테이블 자동화 작업 등의 업무를 주로 맡는다. 
	- 주요 언어: 주로 Scalar, Java, Python 등의 언어를 숙지하고 있으며, 경우에 따라 웹/앱/서버 개발을 진행하기도 한다.
- 데이터 분석가
	- 지표 정의 및 데이터 탐색 분석, 모델링 업무를 맡는 경우가 많다. 통계/수학이나 산업공학을 전공한 경우가 많으며, 경영/마케팅, 사회/심리, 물리/천문학 등 다양한 전공자들이 데이터 분석 업무를 수행한다. 한 분야의 전문성과 지식으로 복잡한 현상을 해석하기에 한계가 있으므로 여러 관점을 통해 다각적이고 풍부하게 해석하고자 하는 의지가 반영된 결과라 볼 수 있다. 
	- 주요 언어: 최근 데이터의 크기가 커짐에 따라 단일 머신으로 처리하기가 거의 불가능하다. 따라서 분산 처리가 가능한 Spark를 사용하는 추세이며, Spark가 제공하는 R, Python API를 이용해 분석 및 모델링을 하는 경우가 많다. 데이터 추출시에는 SQL를 활용한다. 데이터 크기가 작은 경우 엑셀, SPSS 등을 활용하기도 한다.
