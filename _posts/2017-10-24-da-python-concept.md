...
#### 데이터 처리
엔지니어가 담당하는 데이터 처리와 분석가가 진행하는 데이터 처리는 차이가 다소 있다. 엔지니어는 주로 실시간 혹은 시간대별 배치 작업을 통해 테이블을 업데이트하거나 동기화하는 업무를 맡는다. 이러한 작업 덕분에 분석가는 원하는 데이터를 추출하여 분석전에 전처리 작업을 진행할 수 있다. 분석가의 입장에서 전처리 작업은 주로 아래와 같은 활동을 의미한다.
...
#### 리포팅 및 피드백
분석 결과 및 인사이트를 설들력 있게 정리/전달하는 과정이다. 아무리 좋은 분석 결과를 도출했다하더라도 이 단계가 제대로 진행되지 않으면 그 효과가 반감된다. 주로 아래의 원칙이 효과적이고 설득력 있는 전달을 하는 데 유용할 수 있다.
- 내용의 초점은 분석가 아닌 상대방
	- 상대가 이해할 수 있는 언어 사용
	- 상대의 입장에서 생각하고 결과를 정리
	- 목적을 상기, 분석 과정 보다는 결과, 행동 중심으로 전달
- 간결하고 명확한 메시지로 전달
	- 짧은 문장
	- 블랙포인트 방식
	- 핵심 내용 먼저, 이후 근거 나열
...
### 팀구성 및 역할
데이터사이언스팀은 주로 엔지니링과 분석 파트로 구분되며, 서로 협업을 통해 프로젝트를 진행하는 경우가 많다. 엔지니어와 분석가는 보유 역량 및 지식의 영역이 다르기 때문의 원활한 커뮤니케이션과 협업의 중요성이 특히 더 강조된다. 데이터 엔지니어와 분석가의 각 역할은 아래와 같다.

#### 데이터 엔지니어
- 역할: 소프트웨어 개발에 대한 경험과 전문성을 보유하고 있으며, 시스템 환경(Spark, Hadoop) 설치 및 마트 관리와 테이블 자동화 작업 등의 업무를 주로 맡는다. 
- 언어: 주로 Scalar, Java, Python 등의 언어를 숙지하고 있으며, 경우에 따라 웹/앱/서버 개발을 진행하기도 한다.

#### 데이터 분석가
- 역할: 지표 정의 및 데이터 탐색 분석, 모델링 업무를 맡는 경우가 많다. 통계/수학이나 산업공학을 전공한 경우가 많으며, 경영/마케팅, 사회/심리, 물리 등 다양한 전공자들이 데이터 분석 업무를 수행한다. 한 분야의 전문성과 지식으로 복잡한 현상을 분석하고 해석하기에 한계가 있으므로 여러 관점을 통해 다각적이고 풍부하게 해석하고자 하는 의지가 반영된 결과라 볼 수 있다. 
- 주요 언어: RDB에서 SQL을 통해 데이터를 추출하거나, 분산 처리 환경인 경우 Spark가 제공하는 API(R이나 Python, SQL)를 이용해 분석 및 모델링을 하는 경우가 많다. 데이터 크기나 환경에 따라 엑셀, SPSS, SAS, Matlab 등을 활용하기도 한다.

### 분석 툴 및 환경
분석 환경 및 시스템은 회사와 팀의 고유한 상황에 따라 크게 좌지우지되는 경향이 있다. 일반적으로 IT 회사의 경우, 오픈소스 및 신기술에 대한 관심도가 높을 가능성이 있다. 반면에 보안에 민감하거나 보수적인 산업의 기업들은 전통성이 있거나 상용 소프트웨어를 선호하는 경향이 있다. 항상 예외는 있다. 보수적인 IT 회사도 존재하며 유연한 사고의 금융, 제조 회사도 존재한다.    
    
분석 환경 및 시스템의 실제적인 구축과 관리는 엔지니어의 역할이다. 다르게 이야기 하면, 이들의 도움 없이는 분석가 자체적으로 데이터를 추출/처리/분석하기에는 많은 어려움이 따른다. 물론 분석가의 취향대로 환경이 구성되는 경우도 있으나, 이 경우 분석가가 의사결정권자이거나 든든한 지원자가 있을 경우일 것이다. 일반적으로 분석가는 엔지니어가 구성해 놓은 환경/시스템에 맞게 적응한 후 그 환경 안에서 (엔지니어의 도움으로) 분석을 하는 것이 일반적이다.     
이제 IT 회사를 예시로 분석 환경/시스템에 대해 간단히 살펴보자.      
- Spark (오픈 소스)
	- Java로만 다룰 수 있는 Hadoop을 보다 분석 친화적으로 개선 (Java, Scalar, Python, R을 이용해 Hadoop 장점 활용 가능)
	- RDD, DataFrame, Dataset 등 다양한 데이터셋 제공
	- RDD를 이용할 경우 데이터 I/O 작업을 디스크가 아닌 RAM 이용 가능
	- Lazy execution으로 보다 효율적인 작업이 가능
	- Spark에서 Python 및 R의 다양한 라이브러리 사용 가능(Local Dataframe 형식으로 변환후)
- Zeppelin (오픈 소스)
	- 코드를 쓰고 저장하고 데이터를 시각화할 수 있는 노트북
	- 인터액티브 방식
	- 간단한 시각화/대시보드 제공
- 기타 툴
...
### 소프트 역량
#### 문제 정의
분석 프로세스의 첫 단계는 '문제 정의'이다. 이 단계에서 첫단추를 어떻게 맞추는가에 따라 프로젝트의 성패가 갈리는 경우가 많다. 문제의 정의를 잘 하는 방법은 전문 지식이나 특정 스킬이 요구되기 보다, 경험이 더 중요하게 작용한다. 주로 아래와 같은 원칙을 염두하고 문제를 정의하는 것을 추천한다.
- 충분한 시간을 투자해 논의 진행
...
데이터 분석을 진행할 때 분석가는 많은 유혹을 느낀다. 시간을 투자해서 주제에 대해 깊게 들어가보고 싶고, 여러 방법론을 적용하면서 모델의 성능을 높이거나 풍부한 결과를 내고자 하는 생각을 가지게 된다. 그러나 현실적으로 시간은 제한이 되어 있어, 적당한 깊이에서 분석을 중단하는 것이 현명한 경우가 많다.   
   
또, 분석을 진행하다 보면 목적성을 상실하고 분석 행위의 의미를 잃게되는 경우도 있다. 이 때 나침반의 역할을 하는 것이 분석 목적이다. 문서화를 하여 분석 진행중에 상시로 확인하면서 배가 산으로 가는 일을 방지하는 것이 중요하다. 예를 들면, 코드를 쓰는 편집기에 목적을 간단히 기입하는 것이 유용한 방법이다.

#### 프레임 설정
데이터 분석 프로젝트를 시작할 때, (내용이 비어 있는) 틀을 짜고 추후에 데이터 분석을 통해 내용을 채워 넣는 방식으로 진행하는 것이 효율적이다. 사전에 설정한 프레임이 잘못된 방향으로 분석이 진행되는 것으로 예방한다. 더불어 예상 내용을 미리 추측하고 각 결과에 따른 대응 방안을 미리 예상해두면(생각할 시간이 많기 때문에) 예상하지 못한 결과에 대해 당황하지 않고 풍부한 인사이트를 도출할 수 있다. 프레임을 설정할 때 아래 원칙을 고려하면 도움이 된다.
...
다음 장에서는 분석에 사용하는 툴과 환경에 대해 보다 자세히 알아보고 실습을 진행하도록 한다.
